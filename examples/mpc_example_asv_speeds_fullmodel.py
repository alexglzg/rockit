...
current_X = vertcat(0.001, 0.001, 0.1, 0.001, 0.001, 0.1)  # initial state

u_ref = 0.7
r_ref = 0.2
final_X   = vertcat(0, 0, 0, u_ref, 0, r_ref)    # desired terminal state
...
#Tport = ocp.state()
#Tstbd = ocp.state()
ocp.set_initial(x,0.001)
ocp.set_initial(y,0.001)
ocp.set_initial(psi,0.1)
...
ocp.set_der(x, (u*cos(psi) - v*sin(psi)))
ocp.set_der(y, (u*sin(psi) + v*cos(psi)))
ocp.set_der(psi, r)
...
X = vertcat(x, y, psi, u, v, r)
ocp.add_objective(ocp.integral(1.0*(u-u_ref)**2 + 1.0*(r-r_ref)**2))
...
'''ocp.subject_to( -30.0 <= Tportdot )
ocp.subject_to( Tportdot <= 35.0)
ocp.subject_to( -30.0 <= Tstbddot )
ocp.subject_to( Tstbddot <= 35.0)'''
