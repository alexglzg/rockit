...
current_X = vertcat(starting_angle, np.sin(starting_angle), np.cos(starting_angle), 0.001, 0.00, 0.00, y_e, x_1, y_1, a_k, ned_x, ned_y)  # initial state
...
ye_ref = 0.0
final_X   = vertcat(0, sinpsi_ref, cospsi_ref, u_ref, 0, 0, ye_ref, 0, 0, 0, 0, 0)    # desired terminal state
...
Tport_history       = np.zeros(Nsim)
Tstbd_history       = np.zeros(Nsim)
...
X_0 = ocp.parameter(nx)
obs_pos = ocp.parameter(8)
obs_rad = ocp.parameter(4)
...
ocp.add_objective(ocp.integral(5.0*(ye-ye_ref)**2 + 2.0*(sinpsi-sinpsi_ref)**2 + 2.0*(cospsi-cospsi_ref)**2 + 30*(u-u_ref)**2 + 0.1*r**2 + 0.001*Tstbd**2 + 0.001*Tport**2))
ocp.add_objective(ocp.at_tf(10.0*(ye-ye_ref)**2 + 4.0*(sinpsi-sinpsi_ref)**2 + 4.0*(cospsi-cospsi_ref)**2 + 20*(u-u_ref)**2 + 0.2*r**2 + 0.002*Tstbd**2 + 0.002*Tport**2))
...
ocp.subject_to( (obs_rad[0] <= distance1) <= 1000000)
ocp.subject_to( (obs_rad[1] <= distance2) <= 1000000)
ocp.subject_to( (obs_rad[2] <= distance3) <= 1000000)
ocp.subject_to( (obs_rad[3] <= distance4) <= 1000000)

# Initial constraints
X = vertcat(psi, sinpsi, cospsi, u, v, r, ye, x1, y1, ak, nedx, nedy)
...
for i in range(Nsim):
    ...
    Tport_history[i] = Tpsol[0]
    Tstbd_history[i] = Tssol[0]
    #F_history[i]       = Fsol[0]
...
ax4.plot(time_sim2, Tport_history, 'r-')
ax4.set_xlabel('Time [s]')
ax4.set_ylabel('Tport [N]', color='r')
ax4.tick_params('y', colors='r')
ax5 = ax4.twinx()
ax5.plot(time_sim2, Tstbd_history, 'b-')
