...
# Lagrange objective
ocp.add_objective(ocp.sum(Qye*(ye**2) + Qpsi*(sin(psi)-sin(gamma_p))**2 + Qpsi*(cos(psi)-cos(gamma_p))**2 + Qu*(u-u_ref)**2))
ocp.add_objective(ocp.at_tf(Qye*(ye**2) + Qpsi*(sin(psi)-sin(gamma_p))**2 + Qpsi*(cos(psi)-cos(gamma_p))**2 + Qu*(u-u_ref)**2))

# Path constraints
ocp.subject_to( (-1.4 <= u) <= 1.4 )
...
for i in range(Nsim):
    ...
    ocpf.set_value(X_0, vertcat(current_X[:8],s_0))
    # Solve the optimization problem
    sol = ocpf.solve()
    #ocpf._method.opti.set_initial(ocp._method.opti.x, ocp._method.opti.value(ocp._method.opti.x))
