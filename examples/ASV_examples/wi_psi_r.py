...
def path_w_args(s_var, xpos, ypos):
    return ((ypos-((y_multiplier*s_var)+y_start))**2 + (xpos-(x_amplitude*np.sin(s_var*x_freq)+x_start))**2)
s_0 = minimize(path_w_args, 0, method='nelder-mead', args=(ned_x, ned_y), options={'xatol': 1e-8, 'disp': True})
s_0 = s_0.x
print(s_0)

current_X = vertcat(ned_x,ned_y,starting_angle,u_ref,0,0,starting_angle,s_0)  # initial state
...
psi_d = ocp.register_state(MX.sym('psi_d', 1))
...
s_min = ocp.register_control(MX.sym('s_min', 1))
...
# Lagrange objective
ocp.add_objective(ocp.sum(Qye*(ye**2) + Qp*(sin(chi)-sin(gamma_p))**2 + Qp*(cos(chi)-cos(gamma_p))**2 + Qr*(r**2) + R*(Ur**2)))
...
ocp.subject_to( (-4 <= Ur) <= 4 )
ocp.subject_to( s >= 0)
ocp.subject_to( s_min >= 0)

# Initial constraints
X = vertcat(nedx,nedy,psi,u,v,r,psi_d,s)
...
ocpf._method.add_sampler('psi_d', psi_d)
