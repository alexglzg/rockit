...
trajectory = ocp.register_parameter(MX.sym('trajectory', 6), grid='control')

# Lagrange objective
ocp.add_objective(ocp.sum(Q0*(nedx-trajectory[0])**2 + Q1*(nedy-trajectory[1])**2 + Q2*(psi-trajectory[2])**2 
...
for i in range(Nsim):
    ...
    ocpf.set_value(X_0, current_X[:6])
    ...
    ocpf.set_value(trajectory, trajectory_N)
