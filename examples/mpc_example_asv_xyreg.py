...
nx    = 8                   # the system is composed of 14 states
...
ned_x = 0.0
ned_y = 0.0
current_X = vertcat(ned_x, ned_y, starting_angle, np.sin(starting_angle), np.cos(starting_angle), 0.001, 0.00, 0.00)  # initial state

x_ref = 1.0
y_ref = 1.0
final_X   = vertcat(x_ref, y_ref, 0, 0, 1, 0, 0, 0)    # desired terminal state
...
ocp.set_der(psi, r)
ocp.set_der(sinpsi, (cos(psi)*r))
ocp.set_der(cospsi, (-sin(psi)*r))
...
X = vertcat(nedx, nedy, psi, sinpsi, cospsi, u, v, r)
ocp.add_objective(ocp.integral(0.1*(x_ref - nedx)**2 + 0.1*(y_ref - nedy)**2))
...
'''ocp.subject_to( -30.0 <= UTportdot )
ocp.subject_to( UTportdot <= 35.0)
ocp.subject_to( -30.0 <= UTstbddot )
...
for i in range(Nsim):
    ...
    # Log data for post-processing
    x_history[i+1]   = current_X[1].full()
...
ax1.plot(time_sim, x_history, 'r-')
ax1.set_xlabel('Time [s]')
ax1.set_ylabel('X [m]', color='r')
ax1.tick_params('y', colors='r')
ax2 = ax1.twinx()
ax2.plot(time_sim, y_history, 'b-')
